...
| NFR-2026-001 / REQ-2026-006/008 | 锁定项在整图保存/UI 层不被覆盖、可锁定/撤销 | WORK-2026-020 | `save_course_graph` 锁定维度保护；前端四维锁保真 + patch 门锁定 + 撤销回退后端 | `618420c` 实现；`b5e2680` 格式；`c70d339` 首跑修复 | TC-LOCK-001..004 + Web App.lock 2/2；全仓 241/241、Web 22/22；CDP 浏览器端到端（锁定→409→撤销）PASS；职责隔离 QA 待执行 | `docs/USER_MANUAL.md`（待更新） | DB/history Runbook 待建 | implemented |
