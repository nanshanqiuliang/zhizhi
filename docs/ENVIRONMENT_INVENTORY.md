...
| deepseek | openai_chat_completions | fast / quality | deepseek-v4-flash / deepseek-v4-pro | **true**（2026-08-14 owner 批准） | 输入 0.28 / 输出 1.14 | `config/llm/providers.yaml` 快照 + `/models` 探测一致 | `env://DEEPSEEK_API_KEY`（开发 live smoke 用；present，仅环境变量，不落盘） |
