...
- 实际变化（用户反馈：AI 自动绘图排布不均、过于紧凑；要求以上下排布为主）：
  ① 根因一（domain）：`assign_draft_layout` 为"层内全局排序网格"——父不对齐子块
     中心、默认间距小（220/140）、无先修边概念全挤顶行。重写为**垂直 tidy-tree**：
     最长链分层保留；布局父取最深先修父（DAG 多父节点只挂一处，主父链环检测
     防御 LLM 环状关系）；子树宽度递归铺开 + **父居中于子块上方**；无任何先修边
     的概念归**底行**；默认间距放宽 `x_gap 220→260 / y_gap 140→210`。
  ② 根因二（web）：`layoutWorkspace`（「自动排布」）是**硬编码示例坐标**，AI 大图
     点排布后未硬编码节点全挤一行。重写为**通用**垂直树形：任意边第一个入边为
     布局父（与 `parentOf` 语义一致）；BFS visited 防环；锁定节点保持原位且其子
     升为新子树根；孤立节点底行；整体左上归一（40/44）；间距 240/190。
  ③ PNG 导出布局（`png_export.layout_tree`）已满足上下排布，本切片不动。
- 影响模块/接口/schema/migration/prompt：`packages/domain/.../ai_draft.py`
  （`assign_draft_layout` 签名不变、默认参数值放宽）+ `apps/web/src/App.tsx`
  （`layoutWorkspace` 私有函数重写）；无契约/端点/持久化变化。
- 兼容性：既有测试（拓扑分层/锁定保持/示例重载）全绿——新布局对示例图仍为三层
  树形；已保存图坐标不受影响（只在新生成草案或用户点「自动排布」时生效）。
- 验证与证据：红灯（domain 5 failed + Web 3 failed）→ 绿灯（domain 6 用例：父居中/
  垂直间距 ≥200/兄弟间距 ≥240/宽兄弟组均匀+父居中/深链垂直直线/底行+确定性+DAG
  单坐标；Web 3 用例：自定义图父居中+均匀展开/孤立底行/锁定保持+自由子树）；
  pytest **521 passed + 6 skipped**、`pnpm check` **21 文件 / 79 测试**、mypy strict
  45、validator/ruff/build/contracts/peers 全绿 + CI；QA `TR-20260816-005` PASS。
- 性能/安全/运维影响：≤ 数百节点毫秒级递归；无安全面变化。
- 回滚：回退本提交即回旧布局。
- 遗留风险与下一步：PNG 导出布局与画布布局算法尚未统一（两者观感接近，属可选
  重构）；水平/径向布局模式为后续可选项。
