...
related_ids: [REQ-2026-006, REQ-2026-010, NFR-2026-002, ADR-0001, WORK-2026-004, WORK-2026-005, WORK-2026-016, WORK-2026-017, TR-20260814-005, TR-20260814-008, TR-20260814-009]
...
- [ ] AC-1：`GET .../resources/{rid}/file` 返回 PDF 二进制（正确 Content-Type），缺失资源 404、漂移 `source_changed`、storage_key 越界拒绝。
- [ ] AC-2：Web 用 PDF.js 渲染指定页为 canvas；翻页/跳锚点页切换渲染。
- [ ] AC-3：带 `page_bbox` 的锚点在被跳转页显示高亮框（位置按 bbox_norm × 页面尺寸）；无 bbox 锚点只定位页不画框。
- [ ] AC-4：锚点目录点击 → 渲染页 + 高亮；漂移/缺失明确提示不误跳。
- [ ] AC-5：集成/组件/安全测试覆盖正/负路径；全仓门通过。
- [ ] 错误和恢复路径：PDF 加载失败显示可见错误，不影响既有编辑；file 端点失败不污染数据。
- [ ] 回滚/禁用方法：回退本工作项提交可回到页文本查看器；不影响既有持久化/导入/跳转证据。

## 验证计划

| Test ID | 层次 | 场景 | 期望 | 证据 |
|---|---|---|---|---|
| TC-RENDER-001 | integration | file 端点 | 二进制/404/source_changed/越界拒绝 | 红灯→绿灯 |
| TC-RENDER-002 | component | PDF.js 渲染页 | canvas 生成、翻页/跳锚点 | 红灯→绿灯 |
| TC-RENDER-003 | component | bbox 高亮 | 高亮框位置正确、无 bbox 不画框 | 红灯→绿灯 |
| TC-RENDER-004 | security | 漂移/缺失/越界 | 稳定错误，不误跳不泄漏 | 红灯→绿灯 |
| TC-REPO-001 | repository | 全仓门 | validator/Ruff/mypy/pytest/Web | 绿灯 |

## 交付物与关闭

- Commit/PR：分支 `feature/WORK-2026-018-pdfjs-render`；先提交失败测试，再实现最小渲染。
- Contract/ADR/migration/prompt：无新 canonical contract/migration；新增 file 端点；无 prompt。
- Test Run：integration + component + 全仓门按 DoD；职责隔离 QA 对冻结 SHA 复核；浏览器 e2e（渲染/高亮）。
- Release：无托管发布；本地 API + Web 可演示 PDF.js 渲染与高亮。
- 观察结果：本轮完成第 5 步最后一个主要产物；第 5 步可标记 100%。
