...
related_ids: [REQ-2026-006, REQ-2026-010, NFR-2026-001, NFR-2026-002, ADR-0001, WORK-2026-004, WORK-2026-005, WORK-2026-013, WORK-2026-014, TR-20260814-005, TR-20260814-006]
target_stage: "阶段 1 / 自然语言第 5 步"
risk: high
created_at: 2026-08-14T08:55:00+08:00
updated_at: 2026-08-14T08:55:00+08:00
```
...
- [ ] AC-1：schema v2 migration 可把 v1 库升到 v2（新增 resource/resource_version 表），重复/乱序迁移稳定失败。
- [ ] AC-2：导入合法 MD/TXT/PDF 后在受控目录生成副本、`resource` 表有记录（content_hash/mime/byte_size 正确）；重复导入同 hash 返回幂等结果不重复落盘。
- [ ] AC-3：白名单外类型（如 .exe）、伪造扩展名、超大文件（>25 MiB）、路径逃逸文件名均以稳定错误拒绝且不落盘。
- [ ] AC-4：`GET /api/workspaces/{id}/resources` 返回元数据列表（不含内容）；缺失 workspace 404。
- [ ] AC-5：Web 提供导入控件与资源列表展示；错误提示可见。
- [ ] AC-6：集成/安全/组件测试覆盖正/负路径；全仓门通过。
- [ ] 错误和恢复路径：导入失败保留原数据不受影响；磁盘写入失败以稳定错误返回。
- [ ] 回滚/禁用方法：回退本工作项提交可回到 v1 库（迁移前备份）；不影响既有持久化证据。

## 验证计划

| Test ID | 层次 | 场景 | 期望 | 证据 |
|---|---|---|---|---|
| TC-IMPORT-001 | integration | migration v1→v2 | 表存在、版本正确、冲突失败 | 红灯→绿灯 |
| TC-IMPORT-002 | integration | 导入 MD/TXT/PDF | 受控副本 + 元数据正确；重复幂等 | 红灯→绿灯 |
| TC-IMPORT-003 | security | 白名单外/伪造/超大/路径逃逸 | 稳定拒绝且不落盘 | 红灯→绿灯 |
| TC-IMPORT-004 | integration | resources 列表 | 元数据不含内容；404 | 红灯→绿灯 |
| TC-IMPORT-005 | component | Web 导入控件与列表 | 导入成功/失败提示可见 | 红灯→绿灯 |
| TC-REPO-001 | repository | 全仓门 | validator/Ruff/mypy/pytest/Web | 绿灯 |

## 交付物与关闭

- Commit/PR：分支 `feature/WORK-2026-016-safe-import`；先提交失败测试，再实现最小导入。
- Contract/ADR/migration/prompt：schema v2（PRAGMA user_version=2 + 新表）；无新 canonical contract/prompt。
- Test Run：integration + security + component + 全仓门按 DoD；职责隔离 QA 对冻结 SHA 复核。
- Release：无托管发布；本地 API + Web 可演示导入。
- 观察结果：本轮交付"资料进入工作区"的安全入口；PDF 解析、查看器、来源跳转属于第 5 步后续工作项。
