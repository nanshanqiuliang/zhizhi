...
- 环境/版本/build/config：commit `c878c44`（feature/WORK-2026-047-edit-toolbox）；local-dev
  Windows x64。桌面产物重建：`dist/zhizhi/zhizhi.exe`（8,632,729 字节，13:36:36）、
  `dist/zhizhi-0.1.0-setup.exe`（19.3MB）、`dist/zhizhi-0.1.0-portable.zip`（22.5MB）。
- 变更或症状：① 拖拽节点松手后画布跳动（浏览器补发 click 触发重定心）→ 已修复；
  ② 编辑功能不完整（仅「添加子概念」）→ 新增「添加概念」「添加总纲」「连线」模式与
  详情面板「关联关系」断线；③ 边类型保存时被改写为 related_to → 往返保留。
- 影响：用户需安装新产物后生效；数据目录不受影响；重建时发现用户正在运行的旧实例占用
  dist 文件，已关闭该实例后重建（数据自动保存，重开恢复）。
- 证据：QA `TR-20260815-009` PASS（correlated_review，0 P0/P1；1 P2 既有 BUG-2026-001）；
  Web 64/64、pytest 469/469 + 5 skipped、e2e 18/18、后端边类型闭环 6/6。
- 缓解/回滚：回退 `c878c44` 即回旧行为（含边类型改写缺陷）。
- 遗留风险/Owner/期限：空工作区空态兜底（BUG-2026-001）待立工作项；live DeepSeek 全库
  生成复测待 owner key。
...
- 环境/版本/build/config：commit `954a7c8`（feature/WORK-2026-009-ai-draft-pipeline）；local-dev Windows x64。
- 变更或症状：历史记录增 `source`（manual/ai_draft/ai_command，向后兼容）；`accept_ai_draft`→ai_draft、`/interpret/accept`→ai_command、其余 manual；`GET /history` 返回 source；Web 版本历史面板「AI」标记；无迁移/无部署变化。
