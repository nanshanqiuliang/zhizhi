...
面向用户的自然语言阶段、可见里程碑和“继续推进”报告格式见 [知识树笔记 App 自然语言开发路线](USER_FACING_DEVELOPMENT_ROADMAP.md)。第 1 步的可回滚开发默认值已通过 QA；当前第 2 步约 70%，个人可用 MVP 粗略完成度约 20%：Anchor/GraphPatch 纯领域 prototype 的首轮 QA 发现运行时 schema 文件 I/O 与测试计数问题，`1278e79` 红灯复现后由 `5ff02a4` 修复；当前 50 个专项测试、4 个仓库集成测试、全仓 Python 136/136 和 Web 1/1 通过，待职责隔离 QA 复审。百分比只反映已提交、可验证的产品能力，不以计划或测试数量代替功能完成度。
...
| WORK-2026-005 | 冻结 Anchor/GraphPatch v1 | 进行中（QA 修复待复审） | 总工程师 | WORK-2026-002 离线默认值 QA 已通过；正式阶段出口仍待 owner | JSON Schema、纯领域 validator、生成 TS enum/Python runtime artifact、ADR | `44b6233` 原始红灯；`a25470c` QA FAIL；`1278e79` I/O 红灯；`5ff02a4` 修复；专项 50/50 + 仓库集成 4/4、全仓 Python 136/136、Web 1/1、全门通过；QA 复审待执行 |
...
| 项目 | 原因 | 解除条件 |
|---|---|---|
| 后续技术尖峰 | WORK-2026-002 修正已通过 QA；完整架构验收、Anchor/GraphPatch QA 复审、预算、真实 API Key 仍未完成 | WORK-2026-005 继续只推进离线 schema/validator/tests 与证据收口；不得直接跳到 UI、数据库或 live |
