...
| WORK-2026-036 | 画布拖拽修复（仅左键按住时拖拽） | 已完成（QA `TR-20260815-004` PASS） | web | WORK-2026-003 已验证 | `moveDrag` buttons 校验 + `setPointerCapture` + `onPointerCancel` | `69d0de9` 实现；Web 43/43 |
| WORK-2026-037 | 打开本地资料目录（reveal） | 已完成（QA `TR-20260815-004` PASS） | api + web | WORK-2026-016 已验证 | `POST /resources/open-dir` + `/{rid}/reveal`（explorer + 路径守卫）+ Web 按钮 | `1f01795` 实现；集成 2/2 |
| WORK-2026-038 | AI 接入设置（DeepSeek key 配置 + 对话框） | 已完成（QA `TR-20260815-004` PASS） | api + web | WORK-2026-008/026 已验证 | `ai_config.py` + `GET/PUT/DELETE /api/settings/ai` + Web 设置对话框/徽标 | `abf2d9a` 实现；集成 2/2 + Web 2 用例 |
| WORK-2026-039 | 多课程 / 新建课程（多工作区） | 已完成（QA `TR-20260815-004` PASS） | api + web | WORK-2026-013/014 已验证 | `GET/POST /api/workspaces` + `httpPersistApi(baseUrl, workspaceId)` + 侧边栏课程列表/新建/切换 | `e0fc05f` 实现；集成 2/2 + Web 2 用例 |
| WORK-2026-040 | 拖拽背景稳定（第 10 步后修复 1） | 已完成（QA `TR-20260815-005` PASS） | web | WORK-2026-036 已验证 | `selectNodeKeepCamera`：拖拽起点不再重定心视图 | `3cb1734` 红灯；`992af8b` 实现 |
| WORK-2026-041 | 导入保留原文件名与扩展名 | 已完成（QA `TR-20260815-005` PASS） | api + storage | WORK-2026-016 已验证 | `_safe_storage_name`（Windows 非法字符中性化、保留 stem/suffix）+ `_unique_storage_path`（冲突 `-1/-2`） | `5b7c3e2` 红灯；`93321f5` 实现 |
