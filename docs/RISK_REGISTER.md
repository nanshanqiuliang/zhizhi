...
| RISK-2026-007 | 把 OpenAI 兼容误认为语义完全相同，导致 DeepSeek tool/thinking/JSON 失败 | 高 | 高 | 高 | 400、空 JSON、断流、schema failure 上升 | protocol/profile 分层、capability preflight、TC-LLM-001..009（mock）+ TC-DS-001..005（wire fixture）+ 真实 live smoke 5/5（`d81c574`/`a80f43d`） | 隔离 deployment、回退批准配置、恢复 mock | 待定 | 阶段 0/2 | open |
...
| RISK-2026-009 | 模型下线、别名漂移或能力变化造成静默退化 | 高 | 高 | 高 | `/models` 差异、capability mismatch、eval 漂移 | 显式模型 ID、能力快照、启动探测不自动升级；`/models` 已探测确认 deepseek-v4-flash/pro 与配置快照一致 | 关闭 deployment、回退批准 snapshot | 待定 | 阶段 2/运营 | open |
